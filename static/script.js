    ...
    try {
        ...
        for (const activity of data) {
            ...
            activityEl.innerHTML = `
                ...
                <h6 style="margin:0;margin-top:1vmin;">Type: ${activity.sport_type}</h6>
                <div class="activity-data">
                <div class="time-data">
                <h5>Time Data</h5>
                <p>Active Duration:<strong> ${Math.round(activity.moving_time / 3600)} hour${activity.moving_time > 3600 ? "s" : ""}</strong></p>
                <p>Total Duration:<strong> ${Math.round(activity.elapsed_time / 3600)} hour${activity.elapsed_time > 3600 ? "s" : ""}</strong></p>
                </div>
                <div class="distance-data">
                <h5>Workout Data</h5>
                <p>Distance:<strong> ${(activity.distance / 1000).toFixed(2)} km</strong></p>
                <p>Calories:<strong> ${(activity.kilojoules / 4).toFixed(2)} kcal</strong></p>
                </div>
                <div class="distance-data">
                <h5>Speed Data</h5>
                <p>Avg. Speed:<strong> ${activity.average_speed} km/h</strong></p>
                <p>Top Speed:<strong> ${activity.max_speed} km/h</strong></p>
                </div>
                <div class="heart-data">
                <h5>Heartrate Data</h5>
                ${activity.has_heartrate ? `<canvas id="heartrate-${activity.name}">Show Heartrate Stream</canvas>` : '<p>No Heartrate Data</p>'}
                </div>
                </div>
            `;
